...
const CierreCaja = () => {
  ...
  const handleCloseCashRegister = () => {
    const openCajas = cajas.filter((caja) => caja.estado === "Abierta");
    if (!openCajas.length) {
      toast.info("No hay cajas abiertas");
      return;
    }

    setCajas((prev) => prev.map((caja) => (caja.estado === "Abierta" ? { ...caja, estado: "Cerrada" } : caja)));
    toast.success("Cajas abiertas cerradas correctamente");
  };
